...
//-------------------------ACCESSORS-------------------------------
  ...
  public synchronized String getAllSets() {
	  String setList = "";
	  for(int i = 0; i<arr.size(); i++) {
		  setList+= arr.get(i).getType()+"\n";
	  }
	  return setList;
  }
  
  public synchronized String getAllOptions(String setType) throws AutoException {
	  String optList = "";
	  OptionSet set = getOptSet(findSet(setType));
	  for(int i = 0; i<set.size();i++) {
		  optList+= set.get(i).getDataString()+"\n";
	  }
	  return optList;
  }
//--------------------------FIND----------------------------------------
...
//------------------------CHOICE METHODS----------------------------------------------
  ...
  public void setOptionChoice(String setType, String optionData) throws AutoException {
	  ...
	  if (arr.get(cursor).noChoice()) { //Check if choice exists: return true is choice is null for that Option Set
	      ...
	      choice.add(arr.get(cursor).get(cursor2));
	  }
	  else {
